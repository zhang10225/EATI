...
## 7. 推荐使用什么密码学算法
### 7.1 先给结论：**车端消息面走“标准兼容优先”的 ECC 栈**
- **数字签名（首选）**：**ECDSA P-256 + SHA-256**
  - 原因：这是当前 C-ITS / V2X 生态里最稳妥的工程基线，证书尺寸、签名长度、验签开销都比 RSA 更适合 OBU。
  - 适用：CAM / DENM / BSM 等广播安全消息、伪名证书签发与验签。
- **密钥协商（需要保密信道时）**：**ECDH P-256**
  - 适用：OBU 与 RSU / MEC / 后台之间的管理面会话、补查撤销详情、远程维护信道。
- **摘要算法**：**SHA-256**
  - 适用：证书指纹、消息摘要、透明日志叶子哈希、撤销版本链。
- **对称加密（需要保密时）**：**AES-128-GCM 或 AES-256-GCM**
  - 车端/边缘实时面优先 **AES-128-GCM**（更省资源）；
  - 后台监管与长期归档可选 **AES-256-GCM**。

### 7.2 为什么**不建议**把当前 SDK 的 RSA 方案直接搬过去
- 这个仓库当前 SDK 以 **RSA-2048 + SHA-256** 为主，适合第一版 Agent SDK；
- 但在 VANET 里，**消息广播频率高、邻车多、验签密集**，RSA 的证书/签名体积和验签开销都更吃亏；
- 因此 **VANET 消息面**建议优先采用 **ECC（P-256）**，而不是延续 RSA。

### 7.3 各模块建议的算法栈
| 模块 | 推荐算法 | 原因 |
|---|---|---|
| 车辆伪名证书 / 广播消息签名 | **ECDSA P-256 + SHA-256** | 标准兼容好、签名短、车端负担更低 |
| OBU ↔ RSU/MEC 管理面会话 | **ECDH P-256 + AES-128-GCM + HKDF-SHA256** | 兼顾性能与安全，适合短连接/边缘补查 |
| 撤销摘要 / 透明日志 | **SHA-256 + Merkle Tree** | 实现简单、可验证性强、与现有透明日志思路兼容 |
| 条件解匿 / 身份托管 | **2-of-3 门限密钥或 Shamir Secret Sharing** | 防止单点机构直接还原真实身份 |
| 车端密钥存储 | **Secure Element / HSM 内部生成并持有 ECC 私钥** | 降低私钥导出与复制风险 |
| 长期监管归档 | **AES-256-GCM + SHA-256/384** | 更偏后台长期存证，可接受更高算力开销 |

### 7.4 可选增强，但不建议作为第一阶段主线
- **Ed25519 / X25519**：性能很好，但在传统 V2X 标准兼容性上未必是最稳妥首选，除非你能控制整套闭环生态。
- **BLS / 聚合签名**：适合学术优化方向，但工程复杂度、标准兼容与硬件支持门槛更高。
- **PQC（后量子）**：现阶段不建议放到 OBU 广播消息面；如果要试，优先放到**后台证据包长期签名**或**混合证书试点**。
- **动态累加器 / 更复杂的 cryptographic accumulator**：适合研究撤销压缩，但第一版先用“增量 CRL + 摘要 + 边缘补查”更可落地。

### 7.5 一句话建议
- **第一阶段落地组合**：**ECDSA P-256 + ECDH P-256 + SHA-256 + AES-128-GCM + Merkle 透明日志 + 门限解匿**
- 如果你要的是**现阶段最现实、最容易对接标准和车规硬件**的组合，这套最合适。
